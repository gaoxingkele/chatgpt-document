...
def run_report_v2_and_docx(
    ...
    def _revise_one(idx: int, ch_title: str, ch_body: str) -> tuple[int, str]:
    ...
    with ThreadPoolExecutor(max_workers=min(num_chapters, 4)) as executor:
        futures = {
            executor.submit(_revise_one, idx, ch_title, ch_body): idx
    ...
    revised_parts = [results[i] for i in range(num_chapters)]
